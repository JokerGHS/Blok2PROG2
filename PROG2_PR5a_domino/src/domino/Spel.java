...
/**
 * Er kunnen meerdere spelers aan het dominospel meedoen (in deze versie 3 spelers)
 * Voorlopig wordt alleen door de computer tegen elkaar gespeeld. 
 * Elke speler krijgt 6 willekeurig stenen uit de pot. Om de beurt proberen de spelers aan 1 van de 2 kanten een steen aan te leggen
 * zodat hij zo snel mogelijk alle stenen kwijt is.
 * Wanneer de speler geen steen kan aanleggen, moet hij een steen uit de pot halen.
 * De winnaar is degene die het eerst al zijn stenen kwijt is.
 * Het spel is ook afgelopen als er 3 keer achter elkaar een speler niet kan aanleggen en als de pot leeg is
 *
 * 
...
public class Spel {
    ...
    /**
     * Constructor for objects of class Spel
     * Het spel krijgt een pot van 28 stenen.
     * 
     */
    public Spel() {
        pot = new Pot();
        ...
        for (int i = 0; i < aantalSpelers; i++) {
            spelers[i] = new Speler(("Speler " + (i + 1))); // i moet plus een 
    ...
    /**
     * Het spel begint met een willekeurige steen.
     * Aan beide zijde kan aangelegd worden.
     * De loop van het spel kan gevolgd worden.
     * Om de beurt wordt een steen aangelegd.
     * Wanneer een speler geen stenen meer heeft is die speler de winnaar
     * Wanneer geen van de spelers een steen op kan leggen of als de pot leeg is, is het spel afgelopen.
     */
    ...
    public int volgendeSpeler(int beurt) {
        beurt++;
        return beurt % aantalSpelers;  // hoe kon je deze vinden???
    }

    /**
     * De meegegeven steen (evt. gedraaid) wordt links of rechts aangelegd. De String gelegdeStenen wordt aangepast.
     * 
    ...
    public boolean legSteenAan(Steen aanlegSteen) {
        ...
        if (aanlegSteen.getZijdeLinks() == steenLinks) {
            steenLinks = aanlegSteen.getZijdeRechts();  // bug steenRechts to steenLinks
